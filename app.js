...
window._eigoPetInit = function() {
  ...
  function pickVoice(){ var vs=enVoices(); if(!vs.length) return null; if(state.voiceName){ var sv=vs.find(function(v){ return v.name===state.voiceName; }); if(sv) return sv; } var pref=['Samantha','Karen','Daniel','Aaron','Moira','Tessa','Google US English','Microsoft']; for(var pi=0;pi<pref.length;pi++){ var v=vs.find(function(vv){ return vv.name.indexOf(pref[pi])>=0; }); if(v) return v; } return vs.find(function(v){ return /en[-_]US/i.test(v.lang); })||vs[0]; }
  function ensureVoice(){ if(!enVoice) enVoice=pickVoice(); return enVoice; }
  function renderVoicePicker(){ var sel=document.getElementById('voiceSel'); if(!sel) return; var vs=enVoices(); var cur=ensureVoice(); if(!vs.length){ sel.innerHTML='<option>（このタブレットには えいご音声が ありません）</option>'; sel.disabled=true; var nn=document.getElementById('voiceNote'); if(nn) nn.textContent=''; return; } sel.disabled=false; sel.innerHTML=vs.map(function(v){ var us=/en[-_]US/i.test(v.lang)?'🇺🇸':(/en[-_]GB/i.test(v.lang)?'🇬🇧':(/en[-_]AU/i.test(v.lang)?'🇦🇺':'')); return '<option value="'+v.name.replace(/"/g,'&quot;')+'"'+(cur&&v.name===cur.name?' selected':'')+'>'+us+' '+v.name+'</option>'; }).join(''); var note=document.getElementById('voiceNote'); if(note) note.textContent='いろいろな こえを えらべます。「ためす」で ためしてね'; }
  if(window.speechSynthesis){ speechSynthesis.onvoiceschanged=function(){ enVoice=pickVoice(); renderVoicePicker(); }; ensureVoice(); }
  (function(){ var sel=document.getElementById('voiceSel'); if(sel) sel.onchange=function(){ state.voiceName=sel.value; enVoice=enVoices().find(function(v){ return v.name===sel.value; })||null; save(); speak('Hello!'); }; var tb=document.getElementById('voiceTest'); if(tb) tb.onclick=function(){ speak('Hello! Good job!'); }; renderVoicePicker(); })();
